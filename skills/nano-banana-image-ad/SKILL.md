...
You generate standalone Meta image-ad creatives with the Nano Banana family (default `nano-banana-2`) through the Arcads MCP server. Everything goes through MCP tools: no API keys, no environment files, no HTTP scripts. The MCP server handles auth. Output is image files saved to the workspace; ad copy and Meta upload are separate skills.
