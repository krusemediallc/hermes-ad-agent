...
This is the account-audit skill's reference for capturing an ad account's real settings exactly enough that they can be rebuilt faithfully: campaign settings, ad set targeting objects, placements, bidding, attribution, ad tracking, and creative specs including the Advantage+ creative enhancements that are opted in or out. The audit's "Rebuild specs" step reads the field lists below, writes the snapshot files described in section 2, and records the tier used and the gaps left. Sections 6 and 7 describe how a captured spec is meant to be reused so the audit captures what a rebuild actually needs; the audit itself never writes to Meta.

**Token hygiene (Tier A).** The Route B token lives only in the workspace-root `.env` as `ACCESS_TOKEN`. Read it into the shell (`source .env`) and pass it as `$ACCESS_TOKEN` inside the command; never echo it, log it, print it, or paste it into chat, memory files, snapshot files, or job prompts.
...
| **B: Meta Ads CLI `get --fields`** | `meta ads <campaign|adset|ad> get <ID> --fields <list> --output json`; creatives via `meta ads creative get <ID> --output json` | Complete for campaigns, ad sets, and ads (`--fields` passes Graph field names through). Creative: `get` returns the creative object; whether it includes `degrees_of_freedom_spec` and `asset_feed_spec` is UNVERIFIED, so fall back to Tier A for creatives | Route B |
...
**Tier A command template (GET only, run from the workspace root):**

```
source .env
curl -s "https://graph.facebook.com/v25.0/<ENTITY_ID>?fields=<comma-separated field list>&access_token=$ACCESS_TOKEN"
```

Entity IDs come from the primary backend's list calls (MCP `ads_get_ad_entities` or CLI `meta ads <resource> list`); Tier A then reads each entity by ID with the field list from section 3. The audit only ever issues GET requests on this tier. The token must never be echoed, logged, or pasted into chat, memory files, or job prompts.
