...
Validated, parameterizable prompt templates for generating standalone Meta ad creatives. Pair each with a brand product reference image (passed via `referenceImages` on the Arcads MCP generation tools) for brand fidelity.

This library ships with the **image-ad-clone** skill, which owns it and appends new entries. The **chatgpt-image-ad** and **nano-banana-image-ad** generator skills read it before composing any prompt.
...
**The executing skill appends three always-on safety suffixes** to every prompt (you do not need to repeat these constraints in a library entry). Full text in [safety-suffixes.md](safety-suffixes.md):
...
2. **`SAFE_ZONE_SUFFIX`**: forces all text, headlines, CTAs, and focal subjects to fit within the central 84% of the canvas (8% padding from every edge). Backgrounds may bleed; text and focal elements may not. Eliminates clipped headlines.
...
Together: about 1,575 characters of always-on guard, leaving headroom under each model's prompt cap for your actual prompt body.

**Reference image:** unless noted otherwise, pass a clean product hero shot (white-background product photo of the brand's packaging: pouch, bottle, canister, sachets). The reference is the brand-fidelity anchor; the prompt is the format.

**Model portability:** every entry includes a `Model notes` block explaining how the template renders on the two backends the image-ad skills target:

- **`gpt-image-2`** (ChatGPT Image 2): strong text fidelity, dense small-text rendering, faithful UI mimicry (iOS, Slack, Google search, comparison tables). Best for typography-heavy templates. Max 5 reference images.
- **`nano-banana`** (Nano Banana 2, Gemini Flash Image): strong photorealism, multi-image reference blending, character continuity across runs. Weaker on dense small text. Up to 14 reference images.
...
## Aspect ratios (read before picking a template)

The Arcads MCP image tools accept **`1:1`, `16:9`, and `9:16` only**. Several templates were designed for `2:3` or `4:5` feed-portrait canvases; their **Aspect ratio:** field records the design intent. When the recommended ratio is not in the supported set:

1. Generate at the nearest supported ratio (`1:1` for near-square designs, `9:16` for tall designs).
2. Crop downstream to the target ratio before upload (note the crop when handing off to meta-ad-launcher).
3. Recompose region percentages if the fallback ratio is much shorter or taller than the design intent.

## How to use this file

When the user asks for an image ad with a known format, find the matching template below, check the aspect-ratio note above against what the MCP accepts, then replace the `{placeholder}` variables (pull brand values from BRAND.md) and invoke the matching chatgpt-image-ad or nano-banana-image-ad skill with the filled prompt plus the brand's reference image.

**Variable conventions:**
- `{brand.name}`: wordmark text on the product label (e.g. `VerdantOne`)
- `{brand.color_primary}`: primary brand color hex (e.g. `#143D2B` deep forest green)
- `{brand.product_description}`: one-line how-the-product-looks (e.g. `forest-green pouch with white VerdantOne wordmark, clear shaker bottle with green liquid, coral travel sachets`)
- `{brand.competitor_category}`: what the brand is replacing or winning against (e.g. `Generic Multivitamin`)
- `{brand.tagline}`: the brand's short value prop (e.g. `Greens, simplified.`)
- Template-specific variables defined per section.

The seeded example fills use VerdantOne Daily Greens (a fictional greens-powder brand) as the test case throughout; always swap in the user's brand from BRAND.md.
...
**Aspect ratio:** `2:3` design (generate `1:1` or `9:16` and crop)

**Reference image:** clean product hero (helps brand-color drift in the rare emoji that gets a brand cue)

**Variables:**
- `{notes_title}`: bold black title (40 chars max, may end with a brand-relevant emoji)
- `{date_string}`: small grey timestamp under the header (e.g. "March 14, 2026 at 6:42 AM")
- `{checklist_items[]}`: 8-12 items, each conversational plus 1-2 emoji decorations, optional date suffix
...
**Example fill** (VerdantOne):
- `{notes_title}` = `Why I Switched To VerdantOne 🌿`
...
**Variables:**
- `{publication}`: publication wordmark (`THE MORNING LEDGER`, `NEW LEAF REVIEW`, `Wellness Weekly`, `The Greenfield Journal`)
- `{headline}`: bold editorial headline, 8-15 words, declarative
- `{subcopy}`: 1-2 sentence editorial sub-copy expanding the headline
- `{photo_subject_description}`: what's shown in the bottom 55% (e.g. `fit person in athletic wear holding clear VerdantOne shaker, VerdantOne pouch + canister + sachets in foreground`)
- `{tagline}`: short brand promise on the bottom band (4 words max, e.g. `Greens, simplified.`)
- `{band_color}`: brand-color hex for the bottom band (e.g. `#143D2B`)
...
**Example fill** (VerdantOne):
- `{publication}` = `THE MORNING LEDGER`
- `{headline}` = `How VerdantOne Became the Greens Powder Doctors Actually Recommend`
- `{subcopy}` = `Most multivitamins fail bioavailability tests. This brand built theirs around what your body can actually absorb.`
- `{tagline}` = `Greens, simplified.`
- `{band_color}` = `~#143D2B`

**Model notes:**
- **gpt-image-2:** clean. Publication wordmark legibility is strong; subhead reads at small size.
- **nano-banana:** clean for the photograph block; weaker on the subhead text legibility at 1:1. Consider scaling the subhead text up.
...
- `{ugc_subject}`: what the UGC photo shows (e.g. `fit woman in workout clothes mid-laugh holding an VerdantOne shaker bottle`)
...
- **nano-banana:** mixed. The UGC photo block looks great, but tweet text legibility drops. Consider hybrid: nano-banana to generate the UGC photo asset separately, then composite via gpt-image-2 for final assembly.
...
**Reference image:** clean product hero with multiple SKUs visible (the 2x2 grid uses 4 product variations)
...
- `{publication_logos[]}`: 4 publication wordmarks for the bottom row (e.g. `THE MORNING LEDGER`, `Wellness Weekly`, `Peak Habits`, `The Greenfield Journal`)
...
- **nano-banana:** publication wordmarks drift, letters get re-spaced. Pass each publication as a reference image if pixel-perfect logo accuracy matters.
...
- `{competitor_image_description}`: generic stand-in product (1 line, deliberately dull and unbranded)
...
- **nano-banana:** clean structurally but the small row labels can lose sharpness. Consider 1:1 with fewer rows if using nano-banana.

---

## T6 - Comparison table (dark, hooky)

**When to use:** stop-the-scroll dark-mode creative. More provocative tone than T5, leads with a "this RUINS X" hook. Strong for SaaS, info-product, and supplement brands willing to be confrontational with the category.
...
- **nano-banana:** clean for the headline and "VS" tile row; small numbers in the table can blur. Reduce the table to 4 rows for nano-banana.
...
- `{product_unit_description}`: what gets scattered around the note (e.g. `coral / orange-red VerdantOne travel sachets with white "VerdantOne" wordmark`, `green sour gummy worms with sugar crystals`, `single-serve protein scoops`)
...
- `{product_unit_description}` = `VerdantOne travel sachets: small rectangular foil packets in warm coral / orange-red color, with white "VerdantOne" wordmark visible on each, stamped boldly`
...
**When to use:** UGC-aesthetic "real person made this" social proof. Strong for DTC, supplements, coaching and info-products. Especially effective in IG Reels-derived ads.

**Aspect ratio:** `2:3` design; a tall held-up board needs vertical room (generate `9:16`; `1:1` clips the headline at the top)

**Reference image:** product hero (informs the brand color used for the "wins" column marker)

**Variables:**
- `{N}`: number of reasons (default 5)
- `{brand.name}`: wordmark for the LEFT (winning) column (e.g. `VerdantOne`)
...
**Template prompt** (VerdantOne-validated example; swap brand-specifics per the Variables above):
```
...
Top of board, large hand-lettered title centered (this title MUST be fully visible, its top at least 8% below the canvas top, never clipped):
"5 Reasons Why"
(underlined twice with a hand-drawn line beneath)

Just below the title, in two columns separated by a hand-drawn ">" character:
LEFT column header (deep forest green marker, large, underlined twice): "VerdantOne"
...
**Variables:**
- `{testimonial_quote}`: short pull-quote at the top in italic serif (80 chars max, in quotation marks)
...
Top of the image (~12% height): centered, a row of 5 small black filled stars (★★★★★), and immediately beneath in italic dark navy serif: "I haven't switched supplements in 4 years." in quotation marks (a short testimonial pull-quote, 80 chars max).

Center of the image: a clean editorial product hero: a deep forest-green VerdantOne pouch with white "VerdantOne" wordmark prominent, standing upright slightly tilted forward, with a clear shaker bottle of vibrant green liquid beside it. Subtle soft shadow beneath. The product occupies roughly 45-55% of the frame, centered.
...
Each arrow originates near the relevant area of the product (e.g. label area for "75+ Ingredients", powder zone for "Pre + Probiotics") and curves out to the label text positioned in the white space around the product. Arrows are subtle: thin 1.5pt strokes, slight hand-drawn wobble, with a small arrowhead at the callout end.

Bottom of the image (~10% height): the brand wordmark "VerdantOne" rendered prominently in deep forest-green serif/sans-serif (matching the brand's actual wordmark, in a medium weight) centered.
...
**Model notes:**
- **both: clean.** Arrow rendering is fine on both backends; product hero faithfulness depends on the reference image you pass.
...
- `{product_arrangement}`: what's arranged beside the board (e.g. `VerdantOne pouch upright + clear shaker bottle in front + small wooden scoop + 2 travel sachets casually placed`)

**Template prompt** (VerdantOne-validated example; swap brand-specifics per the Variables above):
```
1:1 static ad creative, 1080x1080, edge-to-edge: a real-world product flatlay-style photograph featuring a physical felt letter board with hand-arranged removable white plastic letters, displayed alongside the VerdantOne product. Standalone ad creative, ready to upload.
...
Beside the letter board, slightly to the right and overlapping its lower edge: the VerdantOne product: a deep forest-green VerdantOne pouch with white "VerdantOne" wordmark prominent, standing upright, with a clear shaker bottle of vibrant green liquid in front of it. A small VerdantOne wooden scoop and one or two coral / orange-red VerdantOne travel sachets rest casually on the counter near the base of the pouch.
...
**When to use:** social proof with response: a real customer asking, brand giving a clean call-to-action answer. Strong for higher-consideration purchases where buyers want to see other buyers ask.
...
- `{user_question}`: the prospect's question (80 chars max)
- `{brand.name}`: brand wordmark
- `{brand.avatar_description}`: what the brand's circular avatar looks like
- `{brand_response}`: multi-line answer with a 3-step numbered list (avoid emoji; see the note below)
...
- Bubble text (plain text only, NO emoji): "Has anyone actually tried VerdantOne? Worth the price?"
...
- Avatar: small circular tile, deep forest-green ~#143D2B background with white "VerdantOne" wordmark in chunky bold sans-serif
- Name above bubble: "VerdantOne" in bold black + a small blue circular verified-checkmark badge beside the name
- Bubble text (plain text only, NO emoji, NO unicode symbols):
  ...
  2. Try VerdantOne risk-free for 30 days
...
Lower half (~50% of the image): a clean product photograph of the VerdantOne product set on a soft warm-cream background. The product shot is FRAMED IN FULL: the VerdantOne pouch (deep forest-green with white "VerdantOne" wordmark) is fully visible from cap to base, alongside a clear shaker bottle of vibrant green liquid (also fully framed) and a small fan of three coral / orange-red VerdantOne travel sachets. The bottom edge of the product photograph leaves comfortable safe-zone padding from the canvas bottom (at least 8% margin). Editorial product photography, soft natural light.
...
**Note:** the appended `GLYPH_SAFETY_SUFFIX` handles both the "exactly N comments" constraint and the "no emoji in body text" rule globally; your prompt only needs to describe the desired content.
...
**Reference image:** brand product (helps the CTA bar pull from brand color)
...
- `{chatgpt_response_short}`: KEEP THIS UNDER 4 short lines. Dense small text garbles (see the note below). Use 3-4 numbered points max, each 1 line.
- `{cta_text}`: bottom-bar CTA (e.g. `Try VerdantOne risk-free for 30 days`)
...
VerdantOne checks all three."

Top-right corner of the ChatGPT response, two small thin grey icons: a thumbs-up outline and a thumbs-down outline.

Bottom section (~12% of the canvas height): a horizontal CTA bar in deep VerdantOne forest green ~#143D2B spanning the full canvas width, containing white sans-serif text "Try VerdantOne risk-free for 30 days" left-aligned, with a small white right-arrow ">" on the right edge.

The composition is the standalone static ad creative ONLY: no surrounding browser or app chrome (no URL bar, no tabs, no ChatGPT sidebar, no input box at the bottom). Just the conversation snippet styled like a ChatGPT exchange, with the CTA bar grafted at the bottom.
...
**Note:** keep the ChatGPT response sparse: 3 short numbered lines + 1 closer line, rendered at LARGE text size. Dense or small body text will garble even with the global `GLYPH_SAFETY_SUFFIX`. Less is more.

**Model notes:**
- **gpt-image-2:** clean. Mimics its own ancestor's UI faithfully: ChatGPT logo + response card render reliably.
- **nano-banana:** mixed. The ChatGPT logo can drift, response body-text legibility drops. **gpt-image-2 strongly preferred for this template.**

---

## T13 - Before/After two-panel split

**When to use:** transformation narratives. Universal pattern: works for fitness, supplements, productivity tools, hair and skin, anywhere a "messy before, clean after" makes the pitch instantly.
...
- Centered below the headline (the bottom 75% of the half): a clean, beautifully-styled flat-lay photograph of a single deep forest-green VerdantOne pouch (white "VerdantOne" wordmark prominent), a clear shaker bottle of vibrant green liquid, and three coral / orange-red VerdantOne travel sachets neatly fanned out. Bright natural daylight, premium wellness aesthetic, calm and minimal. The contrast with the left side's chaos is the entire visual joke.
...
**Reference image:** product hero (used for color cues in the CTA pill)

**Variables:**
- `{channel_name}`: Slack channel (e.g. `#wellness-team`, `#growth`, `#founders`)
- `{messages[]}`: 3 messages, each `{author_name, role, time, message_text}`. Avoid emoji in message text.
- `{cta_text}`: pill button text (e.g. `TRY VERDANTONE →`)
...
- Below the name row: regular black sans-serif text in a single paragraph: "Anyone else trying that VerdantOne stuff? Day 14 and I have not reached for coffee once."

MESSAGE 2:
- Avatar (left): illustrated cartoon woman with red hair.
- Right of avatar: bold black "Maria (Founder)" + small grey "10:43 AM".
- Below: regular black sans-serif: "Wait, VerdantOne?"

MESSAGE 3:
- Avatar (left): same brunette cartoon as Message 1 (Sarah).
- Right of avatar: bold black "Sarah (Marketing)" + small grey "10:45 AM".
- Below: regular black sans-serif: "Yeah, picked it up after my trainer kept mentioning it. One scoop covers the whole day. Travel sachets are clutch."

Below the third message, with comfortable vertical spacing, centered horizontally and well within the safe zone (at least 10% padding from the canvas bottom): a large pill-shaped CTA button with rounded corners, deep VerdantOne forest green fill (~#143D2B), containing bold WHITE uppercase sans-serif text "TRY VERDANTONE" with a small white right-arrow ">" beside it.
...
**Note:** the appended `GLYPH_SAFETY_SUFFIX` strips emoji from message bodies and enforces the "exactly N messages" count; you do not need to repeat those constraints in the prompt.
...
RIGHT HALF (~45% width): a vertical photo collage that visually represents each ingredient, rendered as a tall stack of small rectangular product photographs flowing into the VerdantOne product itself. Stack reads top to bottom:
...
No additional text, no logos beyond the VerdantOne wordmark on the pouch.
```
...
**When to use:** data-viz framing of the brand's value prop. Reads as objective even though it is editorial. Strong for productivity, wellness, finance, and time-saving brands where the "without" side has many discrete failure modes.
...
- Bottom label: "with VerdantOne" in regular black sans-serif

RIGHT COLUMN, the losing side:
...
- Bottom label: "without VerdantOne" in regular black sans-serif
...
**Variables:**
- `{old_way_steps[]}`: 5 short pain-point steps for the old-way column (each 4 words max). Last step is a "give up" / failure state.
- `{new_way_steps[]}`: 3 short steps for the new-way column (6 words max each). Ends with the brand's outcome.
...
**Template prompt** (VerdantOne-validated example; swap brand-specifics per the Variables above):
```
1:1 static ad creative, 1080x1080, edge-to-edge: a side-by-side hand-drawn-feel flowchart comparing "The Old Way" to "The VerdantOne Way" as two contrasting workflows. Standalone ad creative, ready to upload. Cream / off-white background ~#F5F1EA.
...
RIGHT HALF (~48% width), "The VerdantOne Way":
- Header: a deep VerdantOne forest-green ~#143D2B rounded-rectangle pill at the top with white sans-serif text "The VerdantOne Way"
- Below it: a shorter vertical chain of just 3 rounded-rectangle boxes connected by clean smooth arrow lines pointing down, each in a soft sage / cream tone with darker green border, containing:
  1. One scoop of VerdantOne daily
...
**Variables:**
- `{prompt_question}`: the dialog's two-line subhead, ending with the brand cue (e.g. `Looking to feel like yourself again? Try VerdantOne.`)
...
- `{question}`: the white-card question text (1-2 lines, conversational, 80 chars max)
...
B. Just below the sticker (with a small gap), the answer is rendered as a typewriter-style monospace text overlay on a translucent white panel, multi-line, centered horizontally with comfortable safe-zone padding from the canvas edges:
   "VerdantOne!! One scoop in the morning,
...
**Template prompt** (VerdantOne-validated example; swap brand-specifics per the Variables above):
```
2:3 portrait static ad creative, 1080x1620, edge-to-edge: a real-world tabletop photograph of a paper napkin with a hand-written ink testimonial, set on a casual restaurant or kitchen table. Standalone ad creative, ready to upload.

Setting: a top-down, slightly angled photograph of a wooden or marble cafe table. Centered in the frame: a folded white paper napkin (or a square white napkin laid flat), with a hand-written black ink message scrawled across it in slightly imperfect cursive / casual handwriting. The handwriting is clearly readable but organic: uneven baseline, occasional ink bleed, real-pen-on-paper texture.

The hand-written message reads (verbatim, multi-line, allowing for natural line breaks):

"When I was tired all the time
my friend told me to try VerdantOne -
...
- A small clear glass with the bottom half remaining of a vibrant green liquid (VerdantOne mixed with water).
- A few crumbs of green powder dusted near the napkin edge.
- One coral / orange-red VerdantOne travel sachet (with white "VerdantOne" wordmark) lying just to the side of the napkin, open / empty.
...
8. Soft green fill: "Open VerdantOne box that just arrived" / "4:20 - 5:20 PM"
...
**Variables:**
- `{background_pattern}`: repeating wordmark / branded backdrop (e.g. low-opacity VerdantOne wordmark on forest green)
...
Background: a deep saturated solid color fills the entire 9:16 canvas (use a deep VerdantOne forest-green ~#143D2B). Subtle pattern: faint white "VerdantOne" wordmark text repeats in stylized graffiti / sticker-style across the background at a consistent angle (~15° clockwise rotation), with low opacity (~15%), creating a textured "branded backdrop" feel without being too busy.
...
- A real-feeling UGC selfie photograph: a fit woman in her early 30s with tied-back brown hair, light golden-hour kitchen lighting, smiling slightly while raising a clear VerdantOne shaker bottle of vibrant green liquid up toward her face. She's mid-laugh, casually dressed in a soft athletic top.
...
**Variables:** `{newspaper_masthead}` (e.g. "The VerdantOne Daily"), `{date_string}`, `{across_clues[]}`, `{down_clues[]}`, `{filled_words[]}` (3-4 short crossing words, 5 letters max each so each grid cell stays large enough to render legibly)
...
The newspaper page fills ~75% of the frame, slightly tilted. Cream/beige aged-newsprint texture and crisp serif typography. Subtle warm shadow from morning daylight. Surrounding the newspaper on the wood surface: a small white cup of coffee with steam (top-right corner of frame), a few loose VerdantOne travel sachets (coral/orange-red) scattered along the top and right edges of the frame, and a green VerdantOne pouch tucked into the bottom-right corner.

Newspaper layout, top to bottom:

- Masthead: "The VerdantOne Daily" in classic black serif, large, with a horizontal rule beneath
...
- Faint "VerdantOne" wordmark watermark at the bottom-right corner of the page in light grey
...
**Model notes:**
- **gpt-image-2:** acceptable. A strict 5x5 grid with big letters still requires explicit prompting; even then expect 10-15% iteration to land clean.
...
**Variables:** `{store_header}` (e.g. "VERDANTONE WELLNESS CO.\nverdantone-example.com\n123 Foundation Way"), `{date_line}`, `{benefits[]}` (each `{label, value}` like `ENERGY  FREE`), `{total_line}` (e.g. "ONE GREAT DAY"), `{footer_text}`

**Template prompt** (VerdantOne-validated example; swap brand-specifics per the Variables above):
```
1:1 static ad creative, 1080x1080, edge-to-edge: a top-down photograph of a long thermal-printer cash register receipt, slightly crumpled and curled at the bottom edge, lying on a clean cream/beige flat surface. The VerdantOne product sits to the right of the receipt as a real-world prop. Standalone ad creative.
...
- Header in monospaced black caps: "VERDANTONE WELLNESS CO. / verdantone-example.com / 123 Foundation Way / Tuesday, May 6, 2026"
...
- Itemized list in monospace, each line: benefit name (left) and "FREE" or a number (right):
  ...
  ----------------------------
  TOTAL: ONE GREAT DAY
- Below the total, a faux barcode (vertical black lines) and small footer text "Thank you for prioritizing your day. / verdantone-example.com"

The VerdantOne product (forest-green pouch with white "VerdantOne" wordmark, glass shaker bottle of green liquid, one or two coral travel sachets) sits casually beside the receipt on the right side of the canvas, well within safe-zone padding.
...
- **gpt-image-2:** clean for the monospace text; the receipt photo can feel composited. Either backend works.
...
**Template prompt** (VerdantOne-validated example; swap brand-specifics per the Variables above):
```
2:3 portrait static ad creative, 1080x1620, edge-to-edge: a top-down photograph of a handwritten founder's letter on cream stationery, with the VerdantOne product and a fountain pen as props on a warm walnut wood desk. Standalone ad creative.
...
- Centered: a single piece of cream/off-white stationery (~70% of canvas width, slightly tilted ~3° clockwise), with a printed dark-green VerdantOne wordmark in the top-left corner of the page.
...
of the first 10,000 VerdantOne Next Gen
pouches. Thank you for trusting us.

I built VerdantOne because the supplement
...
Gratefully yours,
  ...
  Sam (signed in casual cursive script)
  Founder, VerdantOne
  verdantone-example.com"

To the right of the letter (lower-right area, partially overlapping the page edge): the VerdantOne product: a deep forest-green VerdantOne pouch standing upright with white "VerdantOne" wordmark visible, alongside one open coral travel sachet beside it.
...
**Note:** both models partially garble handwritten cursive; keep the letter content under 10 lines and accept that 2-3 words may shift. The aesthetic and intent come through; word-perfect handwriting is hard.

**Model notes:**
- **nano-banana:** strong on the desk-photo realism; cursive imperfection feels organic.
- **gpt-image-2:** cursive can feel slightly font-like. Pass an actual handwriting sample as a reference image to anchor the script style.
...
**Variables:** `{brand_age_label}` (e.g. "VerdantOne, 75", using ingredient count as age), `{location_line}` (e.g. "Foundational Nutrition · Here for the long haul"), `{prompts[]}` (2 prompt+answer pairs in Hinge style)

**Template prompt** (VerdantOne-validated example; swap brand-specifics per the Variables above):
```
2:3 portrait static ad creative, 1080x1620, edge-to-edge: a fake Hinge-style dating app profile card with the VerdantOne product as the "match" subject. Standalone ad creative. Soft cream/off-white background ~#F8F5F0 fills the entire canvas.
...
1. Header bar (~6% of card height): bold black sans-serif "VerdantOne, 75" (the brand name + ingredient count as "age"). Below: a small black location pin glyph + light grey text "Foundational Nutrition · Here for the long haul"

2. Hero image (~50% of card height): a clean editorial photograph of the VerdantOne product: a deep forest-green VerdantOne pouch with white "VerdantOne" wordmark prominent, alongside a clear shaker bottle of vibrant green liquid. Soft cream background continuing from the canvas, premium product photography. A small grey heart icon sits in the bottom-right corner of the photo (the Hinge "like a photo" indicator).
...
**When to use:** UGC creator energy. "Influencer holding the product mid-explanation" with chunky white-on-black caption text + a comments overlay sliding up from the bottom. Native to TikTok/Reels.
...
The full canvas is filled with a vertical TikTok-style video frame. Background: a fit, smiling man in his early 30s, casual hoodie, standing in a softly-lit kitchen at golden-hour. He holds a deep forest-green VerdantOne pouch up close to camera with one hand, his face slightly to the side, mid-grin like he's mid-explanation. Cinematic shallow depth of field, the background is softly blurred (kitchen counter, plants, a window).
...
losing my mind: VerdantOne every
...
- "@morningroutine"
- Comment text: "Bro literally same. VerdantOne is a life saver during launches"
...
Centered in the frame: a large rectangular vertical digital billboard (like a premium transit display), occupying ~70% of canvas width and ~80% of canvas height. The billboard has a thin dark frame with a clean lit display face inside.

Inside the billboard display:
- Background: deep VerdantOne forest-green ~#143D2B, edge-to-edge inside the frame
- Top-left corner: small white "VerdantOne" wordmark
...
- Bottom-center: a clean photograph of the VerdantOne product: deep forest-green pouch with white wordmark, a clear shaker bottle of green liquid, a few coral travel sachets fanned out beside them.
- Very bottom: small white URL "verdantone-example.com"

The billboard is brightly lit (slight glow on the surrounding wall), set into a real-world environment with motion-blurred passers-by and atmospheric background. Premium DTC OOH aesthetic: feels like a real high-end subway ad.
```
...
Centered: a single rectangular scratch-off card (~75% of canvas width, slightly tilted ~3° clockwise), thick cardstock with rounded corners, a faint paper texture. The ticket has a deep VerdantOne forest-green border (~5% of card width) framing the central content.

Ticket content, top to bottom:
- Top header bar: white box containing "VERDANTONE PRESENTS" in small forest-green caps, a small green leaf icon, and serial number "No. 075777 / NEXT GEN"
- Title: "Match to Foundational Nutrition" in bold dark forest-green serif (large)
- Centered: a 3x2 grid of six square panels: five of them are silver scratch-off panels (with realistic foil texture, partially scratched away revealing icons + words underneath), one is a clean white panel containing the VerdantOne product photo.
...
6. Bottom-right: clean white panel with the VerdantOne product photo (forest-green pouch with white wordmark, small)

- Below the grid, large bold serif: "WIN ALL FIVE = ONE SCOOP DAILY"
- Footer text in small caps: "$0 LOSING TICKETS. EVERY SCOOP WINS. verdantone-example.com"
...
**Template prompt** (VerdantOne-validated example; swap brand-specifics per the Variables above):
```
1:1 static ad creative, 1080x1080, edge-to-edge: a clean editorial layout with a bold pain-point headline at top, an unchecked checklist of relatable pain points in the middle, the VerdantOne product photo in the lower portion, and a calm tagline + brand wordmark at bottom. Standalone ad creative. Pure white background.
...
Lower section (~32% height): a clean centered editorial photograph of the VerdantOne product: a deep forest-green VerdantOne pouch with white "VerdantOne" wordmark prominent, alongside a clear shaker bottle of vibrant green liquid. Soft natural light, premium product photography.

Bottom section (~14% height): centered, two stacked lines of black text:
- Line 1: bold serif: "ONE SCOOP. ZERO COMPROMISES."
- Line 2: smaller regular sans-serif: "VerdantOne: foundational nutrition for anyone tired of the supplement aisle. verdantone-example.com"
...
Background: deep VerdantOne forest-green ~#143D2B, edge-to-edge, with a subtle radial darker vignette centered on the canvas.
...
Bottom-right corner (~28% of canvas, fully within safe zone): a clean editorial photograph of the VerdantOne product: deep forest-green pouch with white "VerdantOne" wordmark visible, slightly tilted, with a faint soft drop shadow. The product is photographed against the same green background, color-matched.
...
**Template prompt** (VerdantOne-validated example; swap brand-specifics per the Variables above):
```
9:16 portrait static ad creative, 1080x1920, edge-to-edge: a fake iOS Messages app conversation thread with EXACTLY FOUR message bubbles (two incoming grey, two outgoing blue, the last outgoing being a rich link preview card). Standalone ad creative. Pure white background ~#FFFFFF.
...
- Small editorial photograph of the VerdantOne product (forest-green pouch with white "VerdantOne" wordmark + clear shaker bottle of green liquid)
- Below the photo, a thin hairline divider, then small black text:
  Title: "verdantone-example.com"
  Subtitle: "VerdantOne: One scoop. Foundational calm + clarity. Game changer."

BUBBLE 4 (incoming, left-aligned):
- Light grey bubble:
  "omg need!! ordering now"

(Render exactly the four bubbles described above: two incoming grey, two outgoing blue, the last outgoing being a rich link preview card.)
...
**Model notes:**
- **gpt-image-2:** clean. iOS Messages UI mimicry is strong, the rich-link card renders faithfully. **Preferred backend.**
...
**Variables:** `{magazine_title}` (e.g. "VITALITY"), `{issue_subbar}` (e.g. "THE WELLNESS ISSUE · SUMMER 2026 · ISSUE 16"), `{cover_lines[]}` (3-4 cover-line headlines stacked down the left edge, each with a topic + 1-line description), `{spotlight_badge_text}` (short circular-badge text, e.g. "EXCLUSIVE BRAND SPOTLIGHT"), `{bottom_band_text}` (italic serif, 2 lines)
...
CENTER (the hero): a large editorial photograph of the VerdantOne product: a deep forest-green VerdantOne pouch with white "VerdantOne" wordmark prominent, alongside a clear shaker bottle of vibrant green liquid, occupying the center-right of the cover (~50% of canvas width). Soft natural daylight. The product is the visual focal point of the cover.

UPPER-RIGHT CORNER overlay: a small circular badge in dusty rose color, with white serif text inside reading: "{spotlight_badge_text}"
...
The aesthetic is glossy premium magazine: high-end wellness-title feel. Pixel-faithful to a real magazine cover photograph.
```
...
**Variables:** `{surface_color}` (typically charcoal-black or warm walnut), `{items[]}` (6-8 EDC items each with `{description, position, label}`), `{center_caption}` (small two-line product label under the centerpiece, e.g. brand name + tagline)
...
Centerpiece: the VerdantOne product: a deep forest-green VerdantOne pouch with white "VerdantOne" wordmark, alongside a clear shaker bottle filled half-way with vibrant green liquid, sitting roughly center-canvas.
...
Center label, just under the product itself: a small white sans-serif title in two stacked lines: "{center_caption}"
...
- **gpt-image-2:** clean for the annotation labels; the flatlay can feel slightly synthetic.
...
"0% Stress."
"Foundational VerdantOne for founders."
...
Column 1: "9 AM" / sun icon (soft yellow) / "Clear Strategy" / "Mental clarity"
Column 2: "12 PM" / spark icon (soft amber) / "High Energy Front" / "Peak performance"
Column 3: "3 PM" / sparkle icon (soft mint) / "Zero Panic" / "Calm pressure"
Column 4: "6 PM" / sunset icon (soft peach) / "Stress-Free Sunset" / "Energy holds"
Column 5: "9 PM" / moon icon (soft lavender) / "Deep Unplug" / "Rested mind"

(Render the icons as SHAPES, not literal emoji: stylized minimal vector glyphs in soft pastel colors.)

Lower section (~28% height): a clean editorial photograph of the VerdantOne product (deep forest-green pouch with white wordmark + clear shaker bottle of green liquid + 1-2 coral travel sachets), centered, well within the safe zone. Soft natural light, premium product photography.

Bottom (~4% height): centered, small black sans-serif: "VerdantOne · verdantone-example.com"

The composition reads like a calm, designed weather-app forecast widget, but the data is "your day on VerdantOne." Modern minimal aesthetic.
```
...
"Average cortisol reduction during 8 weeks of daily VerdantOne."
...
Right side of the canvas (~40% width, vertically centered): a clean editorial photograph of the VerdantOne product: deep forest-green pouch with white "VerdantOne" wordmark, alongside a clear shaker bottle of green liquid. The product is photographed against a faintly cream background that fades into the white canvas, with subtle drop shadow.

Bottom (~6% height): small black sans-serif "VerdantOne · verdantone-example.com" centered.

Modern data-led editorial design. The big number is the hero; the chart proves it; the product is the deliverable.
```

**Note:** only use claim numbers the user's brand can actually substantiate; the "-37%" here is the seeded example's placeholder, not a real statistic.

**Model notes:**
- **gpt-image-2:** clean. Strong on the stat number + chart axis labels.
- **nano-banana:** the stat number renders fine at giant size; chart axis labels can blur. Acceptable.
...
**Variables:** `{collection_label}` (small-caps curator or collection line above the title), `{placard_title}` (e.g. "VERDANTONE NEXT GEN"), `{placard_subtitle}` (e.g. "Foundational Nutrition, 2026"), `{placard_body}` (2-4 sentence "museum label" describing the artifact in dry institutional voice)
...
Center-left of the canvas: a slim rectangular plinth (warm walnut-wood top, deep matte charcoal sides), about 18% of canvas height, photographed from a slight 3/4 angle. On the plinth, dramatically lit from above: the VerdantOne product: a deep forest-green VerdantOne pouch with white "VerdantOne" wordmark and a small clear shaker bottle of vibrant green liquid beside it. A soft warm spot-light highlight catches the top of the pouch, casting a gentle elongated shadow on the plinth surface.
...
- Top line, small caps black serif: "{collection_label}"
- Title line, black serif (~22pt): "VERDANTONE NEXT GEN"
...
- Bottom right corner of placard, faint italic: "On loan from verdantone-example.com"

Cinematic museum lighting: a soft warm pool of light around the plinth, rest of the canvas falling slightly into ambient light. The composition treats the product as art: quiet, premium, almost reverent. Photographic style is clean editorial, like a real gallery installation shot.
```
...
When a new ad reference is worth turning into a template, use the **image-ad-clone** skill (this library's owner) to:

1. Generate a faithful reproduction with the original reference passed as a reference image (round 1).
2. Strip any chrome from the prompt and re-run (round 2); verify the standalone creative still reads correctly.
3. Identify what's brand-specific vs. structural; replace brand-specific bits with `{placeholder}` variables.
4. Write an example fill against a different brand to verify the template generalizes.
5. Test against **both** target models when possible and write a `Model notes` line for each: where each shines, where each struggles.
...
New entries continue after T39 (T16 and T22 are intentional gaps from the seeded lineage).

## Known rendering limits (mitigated by the always-on suffixes)

- **Dense small text still garbles** even with the glyph-safety suffix, on both gpt-image-2 and nano-banana, especially in chat bubbles, table rows, calendar blocks, and ChatGPT responses. Keep body-text blocks SPARSE: 3-4 short lines max for chat/response bubbles, large readable size. The fix is fewer words, not more rules.
...
- **Tall content + 1:1 canvas = clipping.** Even with `SAFE_ZONE_SUFFIX`, a held-up board, letter sign, or portrait product really benefits from a tall canvas. Pick `9:16` when the focal subject is tall.

**Solved by the always-on suffixes (no longer need per-template callouts):**
- iOS chrome / Sponsored badges / engagement rows leaking in: handled by `NO_CHROME_SUFFIX`.
- Headlines clipping at edges: handled by `SAFE_ZONE_SUFFIX`.
- Emoji in chat bubbles becoming glyph soup: handled by `GLYPH_SAFETY_SUFFIX`.
- Phantom third comment / extra Slack message: handled by the `GLYPH_SAFETY_SUFFIX` count constraint.
