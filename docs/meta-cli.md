...
| Multi-variant flags on the same command | Flexible / dynamic creative | `--images` (max 10), `--videos` (max 10), `--titles` (max 5), `--bodies` (max 5), `--descriptions` (max 5), `--call-to-actions` (max 5). **Each plural flag is repeated once per value** (`--titles "A" --titles "B"`), never several values after one flag; the CLI keeps only the first value otherwise. The pack's standard 5 primary texts, 5 headlines, 3 descriptions fits in one creative. With `--video`, the thumbnail is auto-generated; do not add `--image` unless the user supplied a custom thumbnail |
