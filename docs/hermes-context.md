...
- **Dashboard, messaging channels, cron:** the `terminal.cwd` key in `~/.hermes/config.yaml` (default `'.'`). The `TERMINAL_CWD` environment variable overrides it per invocation.
...
- **Anywhere:** secrets. No tokens, no keys, no `.env` contents. A Meta token belongs only in the workspace `.env` or the shell environment.
