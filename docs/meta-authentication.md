...
- **Name.** This pack uses `META_MCP_TOKEN`. The MCP config references it as `${META_MCP_TOKEN}` and nothing else.
...
- **Rotation.** When the token expires or leaks, generate a new one, replace the secret at the same place, restart the managed app, and re-run the verification layers below. Do not keep the old value anywhere.
...
|---|---|
| A process environment variable (the token, on the managed env UI) | Restart or redeploy the managed app. A running process does not see the new value. |
| `config.yaml` | May hot-reload (`/reload-mcp` in chat or the dashboard's MCP page). Confirm rather than assume. |
...
- Stop. Do not regenerate tokens, do not re-run login, do not edit the config.
- Do not patch Hermes source, `/opt/hermes-agent`, or site-packages.
- Use the Meta Ads CLI route for the operations you need (SETUP.md Step 4, Route B), or wait for an upstream fix in Hermes or the SDK and re-verify after the next Hermes update.
...
Reads that worked start failing with an auth error. Check the recorded expiry. Graph API Explorer tokens expire in about an hour; long-lived tokens in about 60 days. Repeat the creation steps, replace the secret, restart the managed app, re-run the verification layers.
...
- [ ] Config references `${META_MCP_TOKEN}` only; `trust: untrusted`
- [ ] Managed app restarted after the env change
